...
import { issueWarrantyAction } from '@/lib/actions/admin-service-records';

export function IssueWarrantyForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const outcome = await issueWarrantyAction(formData);
      setResult(outcome);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md">
      ...
      {result?.ok && <p className="text-[12.5px] text-[#1E8E5A] mb-4">Warranty issued.</p>}
      ...
      >
        {isPending ? 'Issuing…' : 'Issue Warranty'}
      </button>
    </form>
  );
}
